...
const APP_ONLY = new Set([
  ...
  // ④⑥ 평면도 「현재」 탭 (2026-08-07) — 지시서
  // `docs/DIRECTIVES/2026-08-07-mobile-ux-batch.md` §2부 ④·⑥.
  // 프로토타입의 시간대 축은 **슬롯 탭만** 있다 — 「주장의 재생」 한 겹이라
  // 「사실의 현장」이라는 둘째 겹이 통째로 없다.
  //   floor.nowMode  현재 탭에서 인물 배지를 걷고(주장을 안 그린다) 범례 자리를
  //                  ⑥ 예산(「남은 조사 N회」)으로 바꾼다.
  //                  ⛳ `mapTime` 에 'now' 를 **안 넣는다** — 그 값은 격자도 읽어서
  //                     슬롯 아닌 값이 들어가면 격자의 열 선택이 사라진다.
  //                     별개 플래그(`state.planNow`)라야 두 화면이 서로를 안 건드린다
  //   ⛳ 전 폭 공통이다 — 「데스크톱 무변경」은 모바일 «수리분»의 회귀 방지 조항이고
  //      ④⑤⑥은 기능 신설이라 관할 밖 (2026-08-07 경훈 판정 · 지시서 머리에 박혀 있다)
  'floor.nowMode',

  // ⑤ 평면도 마커 → 하단 시트 (2026-08-07) — 지시서 §2부 ⑤.
  // 프로토타입에는 도면에서 조사로 들어가는 문이 아예 없다(조사는 목록에서만).
  //   planSheet.open         시트 자체
  //   planSheet.done         미조사(조사명·N회 사용·[조사 실행]) ↔ 완료(확보 물증·[조사 기록])
  //   planSheet.canRun       예산 부족이면 실행 버튼 대신 «이유»를 쓴다
  //   planSheet.targetLabel  시신 정밀 검사는 대상이 없다(mode:'none')
  //
  // ⛳ **실행 경로는 안 늘었다** — [조사 실행]이 조사 목록과 같은 `askInvestigate`
  //    를 부르고 같은 확인 문법(「되돌릴 수 없습니다」)을 거쳐 `doInvestigate` 로
  //    간다. 시트는 «렌즈»고, 지시서의 2부 합격 조건이 그것이다.
  // ⛳ **완료 마커가 그동안 죽어 있었다** — `searchable` 이 아니면 빈 함수라
  //    조사를 마친 자리를 눌러도 아무 일이 없었다. 이 시트가 그 자리를 연다.
  'planSheet.open', 'planSheet.done', 'planSheet.canRun', 'planSheet.targetLabel',
