...
- ★ **크라임씬 실험 산출물을 커밋했다** — `engine/cases/practice-room.yaml` (검증 통과).
  스크래치패드에 두면 다음 기계에서 사라지는데, **이 세션의 중심 주장을 다음 기계가
  스스로 확인할 유일한 방법**이다 (`closing-theater.yaml` 을 ⑤검열관 재현 테스트로
  남긴 것과 같은 이유). 파일 머리말에 **무엇을 증명하는지**와 ★지어냄★ 표시의 뜻을 적었다.
  ```bash
  npx tsx src/cli.ts --case cases/practice-room.yaml   # engine 에서
  ```
  ⚠ **말투 본보기가 아니다** — 진술을 원문 요약으로 써서 `voice-check` 최장/최단 **1.29**
  (산장 2.46)로 낮다. **논리 층 증거물**이고 산문 기준선은 `mountain-lodge`·`closing-theater` 다
