...
> ★ **용의자는 언제나 5명입니다** ★ 이것은 설정값이 아닙니다. 규모가 바뀌어도
> 안 바뀝니다 — 진술 정독·진술 격자·관계도·용의자 카드가 전부 5에 맞춰져 있고,
> 오프라인 플레이에서 5가 가장 좋았다는 것이 근거입니다.
> 넷 이하나 여섯 이상으로 만들지 마세요.

| `scale` | 용의자 | 장 | 공란 | 예산 | 소요 |
|---|---|---|---|---|---|
| `daily` | **5** | 2 | 7 | 6~8 | 20~25분 |
| `campaign` | **5** | 5 | 16~19 | 5~6 | 45~60분 |
...
3. **트릭에 허점이 있는가.** 「자살할 사람이 왜 손님을 불렀나」 같은 것.
   없으면 파고들 지점이 없습니다.
   ⚠ **`flaw.text` 는 물음표로 끝나는 의문문이어야 합니다** (2026-08-06 계약).
   엔딩 1막의 「질문」 비트가 *"질문은 하나로 좁혀졌다. <flaw>"* 로 이 문장을
   **그대로** 인용합니다 — 물음표가 없으면 질문이 평서문으로 끝납니다.
   ⛳ **코드가 「?」를 붙이지 않습니다.** 붙이면 표현을 코드가 쥐는 것이고
   (`MANIFESTO §어긋나는 세 가지 꼴` C 부류), 저작이 물음표를 **뜻해서** 쓴 건지
   빠뜨린 건지 구별할 수 없게 됩니다. **저작 계약이지 렌더 장식이 아닙니다.**
