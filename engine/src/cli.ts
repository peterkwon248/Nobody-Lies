...
console.log('\n  주장과 실제 동선')
console.log('  ' + '-'.repeat(50))
if (r.lies.length === 0) console.log('   아무도 거짓말하지 않는다 — 잡아낼 어긋남이 없다')
for (const l of r.lies)
  console.log(`   ${w(l.person, 20)}${l.slots.join(', ')} 에서 어긋남`)

// difficulty 는 기대 회차(탐욕 시뮬 × 1.5) 하나로 판정한다. 그 모델이 거칠어
// 오라클 하한과 탐욕 상한을 나란히 찍는다 — 둘의 부호가 다르면 라벨을 믿지 말 것
