...
import type {
  Action, Blank, BlankLabel, Case, Chapter, Evidence, Fact, Person, Placement,
  Reveal, Text, TimeSlot,
} from './types.js'
...
function placements(raw: unknown, p: Problems, path: string, slots: Set<string>): Placement[] {
  return arr(raw).map((e: Raw, i) => {
    const at = `${path}[${i}]`
    if (!e?.slot) p.add(at, 'slot 이 없다')
    else if (slots.size && !slots.has(e.slot)) p.add(at, `timeline 에 없는 슬롯 '${e.slot}'`)
    // 무대 목록은 아직 스키마에 없으므로 location 은 자유 문자열이다
...
export function parseCase(raw: unknown, source: string): Case {
  ...
  const timeline: TimeSlot[] = arr(c.timeline).map((t: Raw, i) => {
    if (!t?.id) p.add(`timeline[${i}]`, 'id 가 없다')
    if (!t?.label) p.add(`timeline[${i}]`, 'label 이 없다')
    return { id: t?.id, label: t?.label, ...(t?.window ? { window: true } : {}) }
  })
  if (!timeline.length) p.add('timeline', '시간 축이 비어 있다 — presence·시각 공란이 쓸 어휘가 없다')
  const slots = new Set(timeline.map((t) => t.id))
  ...
  const people: Person[] = arr(c.people).map((x: Raw, i) => {
    ...
    return {
      ...
      presence: placements(x?.presence, p, `${at}.presence`, slots),
      // claims 를 적지 않으면 presence 와 같다 = 무고한 사람은 거짓말하지 않는다.
      // 여기서 presence 를 복사해 채우면 안 된다. 검증기가 '주장이 실제와 완전히
      // 일치하는 범인'을 잡아내는데, 그 검사가 undefined 여부에 기대기 때문이다.
      ...(x?.claims ? { claims: placements(x.claims, p, `${at}.claims`, slots) } : {}),
  ...
  if (ex) {
    ...
    if (ex.slot && slots.size && !slots.has(ex.slot))
      p.add('trick.exit.slot', `timeline 에 없는 슬롯 '${ex.slot}'`)
  }
  ...
  return {
    ...
    incident: { kind: inc.kind, subject: inc.subject, description: inc.description },
    ...(c.prose ? { prose: {
