...
export function verify(c: Case): VerifyResult {
  ...
  /**
   * ─────────────────────────────────────────────────────────────
   *  9-18. **프롤로그가 범인을 부르거나 사실과 어긋난다** (2026-08-02 신설)
   * ─────────────────────────────────────────────────────────────
   *
   * §9-7(b)는 프롤로그가 **확보 단어**를 먼저 말하는지만 본다(*"프롤로그는 새 정보
   * 0 이다"*). 그런데 프롤로그가 흘릴 수 있는 것이 단어만은 아니다.
   *
   * ```
   * ⓐ 누설   용의자 중 **범인만** 이름이 나온다        §9-13(도식이 범인만 가리킨다)과 같은 명제
   * ⓑ 모순   「죽었다」고 한 사람이 피해자가 아니다
   * ⓒ 모순   인원수를 **글자로** 말하는데 실제와 다르다   MANIFESTO ❌C — 표현을 코드가 쥔 자리
   * ```
   *
   * ⓒ 가 특히 이 저장소의 부류다. 생성 프롤로그가 *"그곳에는 **다섯** 사람이 더
   * 머물고 있었다"* 를 **상수 문자열로** 쥐고 있다. 용의자 수가 바뀌면 **문장만 거짓이
   * 되고 아무도 모른다** — 07-30의 「아침에 함께 도착했다」와 똑같은 모양이다.
   *
   * ## 실측 (2026-08-02 · 프롤로그가 있는 44건)
   *
   * ```
   * ⓐ 용의자를 부른다  1건(2%) · 그중 범인만 부른다  0
   * ⓑ 죽었다고 한 이름이 피해자가 아니다              0
   * ```
   *
   * ⚠ **등급은 오류다** — §9-13 과 같다(정답 누설이 이 저장소에서 가장 비싸다).
   * 실측이 전부 0 이므로 골든 케이스가 떨어지지 않는다. **회귀 감시다.**
   */
  {
    const ko18 = (x: unknown) => (typeof x === 'string' ? x : (x as { ko?: string })?.ko ?? '')
    const text18 = (c.prologue ?? []).map(ko18).filter(Boolean).join('\n')
    if (text18) {
      // ⓐ 성을 뗀 꼴로도 찾는다 — 한국어 산문은 「한유빈」을 「유빈」으로 부른다(§9-12)
      const shortOf = (n: string) => (n.length > 2 ? n.slice(-2) : n)
      const suspects18 = c.people.filter((p) => p.id !== c.victim)
      const named = suspects18.filter((p) => text18.includes(p.name) || text18.includes(shortOf(p.name)))
      if (named.length === 1 && named[0]!.id === c.culprit)
        errors.push(
          `프롤로그가 용의자 중 '${named[0]!.name}' **한 사람만** 부르는데 그가 범인이다 — ` +
            '읽자마자 답이 반쯤 드러난다',
        )
       ...
       * ⓑ 죽었다고 한 사람은 피해자여야 한다.
       *
       * ⚠ **피해자 이름은 `people` 에 없다** — 스키마가 *"people 은 용의자 목록이다"*
       * 로 강제한다. 정본은 `victimProfile.name` 이고 **선택 항목**이라 없으면 건너뛴다.
       * (처음에 `people.find(id === victim)` 으로 찾았다가 산장에서 피해자 이름이
       * `'chaewon'`(id)으로 인쇄되며 거짓 양성이 났다.)
       *
       * ⚠ **조사를 떼고 비교한다** — `([가-힣]{2,4})[은는이가]?` 는 앞의 그룹이
       * 「윤다인은」을 통째로 먹고 뒤 그룹이 빈 채로 맞는다. 붙여서 잡고 끝의 조사를 벗긴다.
       */
      const vName = c.victimProfile?.name
      const dead = text18.match(/([가-힣]{2,5}) ?이미 숨을 쉬지 않았다/)
      if (vName && dead) {
        const said = dead[1]!.replace(/[은는이가]$/, '')
        if (!vName.includes(said) && !said.includes(shortOf(vName)))
          errors.push(
            `프롤로그가 '${said}' 이 죽었다고 하는데 피해자는 '${vName}' 다 — 산문과 데이터가 다른 사람을 말한다`,
          )
      }

      /**
       * ⓒ 인원수를 글자로 말하는 자리. **숫자가 산문에 박히면 세계가 바뀌어도 안 따라온다.**
       * 「N 사람」 꼴만 본다 — 「두 사람이 보았다」처럼 **부분을 세는 말**도 있으므로
       * **용의자 전체를 가리키는 문형**(`더`·`가 답했다`)일 때만 문다. 좁게 잡는다.
       */
      const NUM: Record<string, number> = {
        한: 1, 두: 2, 세: 3, 네: 4, 다섯: 5, 여섯: 6, 일곱: 7, 여덟: 8, 아홉: 9, 열: 10,
      }
      const whole = text18.match(/(한|두|세|네|다섯|여섯|일곱|여덟|아홉|열) ?사람이 (더|답했다)/)
      if (whole) {
        const said = NUM[whole[1]!]
        if (said !== undefined && said !== suspects18.length)
          errors.push(
            `프롤로그가 용의자를 '${whole[1]} 사람'이라 하는데 실제는 ${suspects18.length}명이다 — ` +
              '산문이 숫자를 글자로 쥐고 있어 세계가 바뀌어도 안 따라왔다',
          )
      }
    }
  }
