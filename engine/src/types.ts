...
/** 진술 격자의 시간 축. 사건마다 다르고, presence·claims 가 이 어휘를 쓴다 */
...
export type Placement = { slot: SlotId; location: LocationId }
...
export type Person = {
  ...
  /** 실제 동선. 진실이며 플레이어에게 직접 보이지 않는다 */
  presence: Placement[]
  /**
   * 진술에서 주장하는 동선. 생략하면 presence 와 같다.
   * 즉 **무고한 사람은 거짓말하지 않는다가 기본값**이고, 거짓말은 명시적으로
   * 적어야 한다. 검증기가 범인 외에는 적지 못하게 강제한다.
   */
  claims?: Placement[]
  /**
   * 진술 원문. **산문가(빌드 타임 LLM)가 채우는 자리다.**
   *
   * 주장의 내용은 `claims`(없으면 `presence`)에서 나오고, 여기에는 말투와
...
export type Case = {
  ...
  /** 진술 격자의 시간 축. presence·claims·시각 공란이 이 어휘만 쓴다 */
  timeline: TimeSlot[]
  people: Person[]
  victim: PersonId
  culprit: PersonId
