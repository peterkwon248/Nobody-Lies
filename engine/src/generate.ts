...
export function generateCase(seed: number): Case {
  ...
  const person = (id: PersonId, i: number) => ({
    ...
    // 거짓말은 범인만. 무고한 사람은 claims 를 적지 않는다(= presence 와 같다)
    ...(id === culprit
      ? { claims: [{ slot: 't0', location: 'hall' }, { slot: 't1', location: 'hall' }, { slot: 't2', location: 'hall' }] }
      : {}),
  })
  ...
  return {
    ...
    scale: 'daily',
    budget: 3,
    incident: { kind: 'homicide', subject: 'victim', description: '밀실이 아닌 실내에서의 사망' },
    prose: { source: 'template' },
    seedTerms: [tool],
    timeline: [
      { id: 't0', label: '전날 밤' },
      { id: 't1', label: '새벽', window: true },
      { id: 't2', label: '아침' },
    ],
