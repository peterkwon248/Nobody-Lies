 ...
 *
 * ## ⛔ 결 규칙 — 프로필 틀에 「말씀드리-」 를 쓰지 않는다 (2026-08-07 · 경훈 정본)
 *
 * **그 동사는 오프너의 소유다.** 오프너는 팔레트 몫(진입의 결)이고 프로필 `first` 는
 * 내용의 결이다. 둘이 같은 낱말을 쓰면 **겹말**이 된다 — 첫 판에서 35편 중 15편(43%)이
 * 이렇게 나왔다:
 *
 * ```
 * ⛔ 기억나는 대로 말씀드리면, 일지에 적어둔 대로 말씀드립니다. 폐관 준비에는 …
 * ⛔ 몇 번을 말씀드렸지만, 처음부터 말씀드리는 게 낫겠지요. 폐관 준비에는 …
 * ```
 *
 * ★ **병은 「완충 절의 존재」가 아니라 「말씀드리-」의 이중 사용이었다**(경훈 판독).
 * V3·V4 는 오프너와 이미 잘 붙는다 — 그래서 완충 절은 **살리고** 겹치는 셋만 개정했다.
 * `voice-check §0` 이 이 계약을 문다. 문안을 고칠 때 그 검사를 먼저 읽어라.
 *
...
export const VOICES: VoiceProfile[] = [
  {
    ...
    first: '{opener} 일지 순서 그대로입니다. {slot}에는 {place}에 있었습니다.',
    move: [
    ...
    // ⚠ 「말씀드리는」 이었다 — ⓑ' 결 규칙이 `first` 셋 말고 **이 줄도** 잡았다.
    // spec 의 *"기록을 짚듯"* 을 그대로 살려 동사만 바꿨다 (경훈 확인 대상)
    flesh: ['순서대로 짚어가는 게 편합니다.', '적어둔 것 밖의 일은 잘 모르겠습니다.'],
  },
  {
    id: 'defensive',
    spec: '먼저 변명부터 깔고 들어간다. 끝은 부탁으로 무른다',
    first: '{opener} 저를 의심할 이유는 없을 겁니다. {slot}에는 {place}에 있었습니다.',
    move: [
  ...
  },
  {
    ...
    first: '{opener} 처음부터 차근차근 가지요. {slot}에는 {place}에 있었습니다.',
    move: [
