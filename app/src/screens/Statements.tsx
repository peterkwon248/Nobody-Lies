 ...
 *
 * ★ 층위는 **주장**이다 ★ — 확정(브리핑·물증)과 섞이면 안 된다. 진술은 따옴표와
 * 인물색으로 표시하고, 게임은 여기서 어떤 판정도 하지 않는다. 볼드·하이라이트·
 * 모순 표시 전부 금지 (`HANDOFF-TO-CODE.md` §0.2). **거짓말하는 사람의 진술도
 * 다른 넷과 완전히 동일하게 생겨야 한다.**
 *
        ...
        <div className="nl-read">
          ...
          <div className="nl-read-card">
            ...
            {(p.statement?.paragraphs ?? []).map((t, n) => (
              <p key={n} className="nl-read-p" style={{ borderColor: color }}>
                {ko(t)}
              </p>
            ))}
